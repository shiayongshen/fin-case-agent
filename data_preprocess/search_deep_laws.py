...
from utility.search_related_laws import LegalSearchEngine
...
def process_csv_laws(batch_size: int = 30): # 新增 batch_size 參數，預設為50
    ...
    # 讀取CSV檔案
    csv_path = os.path.join(current_dir, "processed_cases.csv")
    
    # 檢查檔案是否存在
    if not os.path.exists(csv_path):
        logger.error(f"找不到CSV檔案: {csv_path}")
        # 嘗試其他可能的位置
        alternative_paths = [
            os.path.join(current_dir, "updated_processed_cases.csv"),
            os.path.join(os.path.dirname(current_dir), "updated_processed_cases.csv"),
        ]
        for alt_path in alternative_paths:
            if os.path.exists(alt_path):
                logger.info(f"使用替代檔案: {alt_path}")
                csv_path = alt_path
                break
        else:
            raise FileNotFoundError(f"無法找到 processed_cases.csv 或其他替代檔案")
    ...
    output_path = os.path.join(current_dir, "law_supplement_results_2014_90.json")
