...
⚠️ 【重要】當調用工具時，case_id 會自動使用：{case_id}，您無需擔心。"""
    ...
    #     # ⭐ 新增：構造包含對話上下文的用戶訊息
    #     # 由於 AutoGen 的 GroupChat 無法自動同步完整歷史到 Agent，
    #     # 我們需要在用戶訊息中包含最近的對話摘要
    #     context_summary = ""
    #     if len(self.group_chat.messages) > 0:
    #         # 提取最近的 5 條對話作為上下文
    #         recent_messages = self.group_chat.messages[-5:]
    #         context_lines = []
    ...
    #                 context_lines.append(f"  {msg_name}: {short_content}")
            
    #         if context_lines:
    #             context_summary = "\n【最近對話上下文】\n" + "\n".join(context_lines)
        
    #     # ⭐ 新增：如果 session 中有 current_case_id，也添加到上下文中
    #     current_case_id = cl.user_session.get("current_case_id")
    #     if current_case_id:
    #         case_context = f"\n【當前案例 ID】\n當前正在處理的案例: {current_case_id}"
    #         context_summary = case_context + context_summary
    #         print(f"[ChatManager] 🔍 已在對話上下文中加入 current_case_id: {current_case_id}")
        
    #     # ⭐ 如果處於 constraint_customization 狀態，直接傳遞訊息（無需添加狀態前綴）
    #     if conversation_state == "constraint_customization":
    #         print(f"[ChatManager] 處於自定義約束狀態，將用戶輸入直接傳遞給 constraint_customization_agent")
    #         enhanced_message = message + context_summary
    #     # 如果處於等待用戶輸入的狀態，將用戶輸入直接傳遞給 GroupChat
    #     # ChatManager 中的自動狀態轉換將根據標記決定下一步操作
    #     elif conversation_state in ["waiting_for_legal_content", "waiting_for_case_content", "waiting_for_legal_confirmation", "waiting_for_case_confirmation", "waiting_for_deep_analysis_confirmation"]:
    #         print(f"[ChatManager] 處於等待狀態 {conversation_state}，將用戶輸入傳遞給 GroupChat")
    #         # 添加狀態標記和上下文到消息中
    #         enhanced_message = f"[當前狀態:{conversation_state}] {message}" + context_summary
    #     else:
    #         # 檢查對話歷史中是否有未回應的「等待自定義狀態確認」狀態
    #         waiting_customization_state = False
    #         if self.group_chat.messages:
    #             for msg in reversed(self.group_chat.messages):
    #                 msg_content = msg.get('content', '')
    #                 # 如果找到「等待自定義狀態確認」標記且之後沒有 host_agent 的新回應
    #                 if "[當前狀態:等待自定義狀態確認]" in str(msg_content):
    #                     waiting_customization_state = True
    #                     print(f"[ChatManager] 檢測到對話歷史中存在「等待自定義狀態確認」狀態")
    #                     break
            
    #         if waiting_customization_state:
    #             # ⭐ 只添加狀態前綴，讓 host_agent 的 LLM 判斷用戶意願
    #             enhanced_message = f"[當前狀態:等待自定義狀態確認] {message}" + context_summary
    #             print(f"[ChatManager] 用戶回應自定義狀態確認問題，傳遞給 host_agent 判斷")
    #         else:
    #             enhanced_message = message + context_summary
        
    #     # 檢查是否需要直接處理某些訊息（標記觸發）
    #     direct_response = await self._check_direct_response(message)
    #     if direct_response:
    #         print(f"[ChatManager] 直接處理訊息，返回結果")
    #         return direct_response
        
    #     try:
    #         # 記錄開始時的訊息數量
    #         start_message_count = len(self.group_chat.messages)
            
    #         # 重置中斷標誌
    #         self.is_interrupted = False
            
    #         # ⭐ 關鍵修復：在 initiate_chat 前清除所有 Agent 的歷史
    #         # 這樣所有 Agent 都會從完整的 GroupChat.messages 重新讀取對話歷史
    #         # 解決 Agent 看不到新注入訊息的問題
    #         # print(f"[ChatManager] 正在清除所有 Agent 的本地緩存...")
            
    #         # 先保存 GroupChat.messages，確保在 initiate_chat 後不會被誤清空
    #         saved_messages = self.group_chat.messages.copy()
    #         # print(f"[ChatManager] 已保存 {len(saved_messages)} 條訊息")
            
    #         # for agent_instance in self.agent_instances:
    #         #     if hasattr(agent_instance, 'clear_history'):
    #         #         try:
    #         #             agent_instance.clear_history()
    #         #         except Exception as e:
    #         #             print(f"[ChatManager] ⚠️ 清除 {getattr(agent_instance, 'name', 'unknown')} 的歷史時出錯: {str(e)}")
    #         # print(f"[ChatManager] ✅ 所有 Agent 的緩存已清除，準備開始對話")
    #         print("[ChatManager] 🔄 同步 GroupChat.messages 到所有 Agent 的 chat_history")
    #         for agent_instance in self.agent_instances:
    #             try:
    #                 agent_instance.chat_history = saved_messages.copy()
    #             except Exception as e:
    ...
    #         last_check_count = start_message_count
    #         current_tool_msg = None  # 追蹤當前的工具呼叫訊息
    #         tool_results = {}  # 儲存工具執行結果 {agent_name: result_content}
            
    #         while not chat_task.done():
    #             # 檢查是否被中斷
    #             if self.is_interrupted:
    #                 print("[ChatManager] 檢測到中斷信號，正在停止對話...")
    #                 chat_task.cancel()
    #                 await asyncio.sleep(0.5)  # 給任務一點時間來清理
                    
    #                 # 發送中斷通知
    #                 await cl.Message(content="⏹️ 對話已被用戶中斷").send()
    ...
    #                 print(f"[ChatManager] 發現新訊息: {last_check_count} -> {current_count}")
                    
    #                 for i in range(start_message_count, current_count):
    #                     if i in processed_indices:
    #                         continue
                        
    #                     msg = all_messages[i]
    #                     agent_name = msg.get("name", "unknown")
    #                     # 更健壯地處理 content: 可能是 None、字串 "None" 或其他型別
    #                     raw_content = msg.get("content", None)
    #                     if raw_content is None:
    #                         content = None
    #                     else:
    #                         content = str(raw_content)

    #                     display_preview = content if content is not None else "<None>"
    #                     print(f"[ChatManager] 訊息 [{i}] {agent_name}: {display_preview[:80]}...")
                        
    #                     # 調試：檢查 constraint_customization_agent 的消息
    #                     if agent_name == "constraint_customization_agent":
    #                         print(f"[ChatManager] 🔍 constraint_customization_agent 消息檢測:")
    #                         print(f"   - msg 鍵值: {msg.keys()}")
    #                         print(f"   - content 類型: {type(raw_content)}")
    #                         print(f"   - 完整內容前200字: {content[:200] if content else 'None'}")
                        
    #                     # 跳過使用者輸入
    #                     if agent_name == "user_proxy" and content == message:
    #                         processed_indices.add(i)
    #                         print(f"[ChatManager] 跳過 user_proxy 原始訊息")
    #                         continue
                        

    #                     # === 新增：過濾掉中間 Agent 的 None 訊息 (包含 host_agent 在呼叫工具時的 None) ===
    #                     # 當 content 為 None、空字串或字串 "None"（大小寫不敏感）時，視為空訊息不顯示
    #                     if agent_name in ["search_agent", "code_executor", "host_agent"] and (
    #                         content is None or (isinstance(content, str) and (not content.strip() or content.strip().lower() == "none"))
    #                     ):
    #                         processed_indices.add(i)
    #                         print(f"[ChatManager] 跳過 {agent_name} 的空訊息")
    #                         continue
                        
    #                     # 處理工具呼叫訊息（傳入整個 msg dict 供 _is_tool_call_message 檢查）
    #                     if self._is_tool_call_message(msg):
    #                         processed_indices.add(i)
                            
    #                         # 提取工具名稱
    #                         tool_name = self._extract_tool_name(content if isinstance(content, str) else "")
                            
    #                         emoji = "🔧"
                            
    #                         # 顯示工具呼叫中的訊息
    #                         tool_message = f"{emoji} **正在使用工具**\n\n⏳ 處理中，請稍候..."
                            
    #                         current_tool_msg = cl.Message(content="")
    #                         await current_tool_msg.send()
                            
    #                         for char in tool_message:
    #                             await current_tool_msg.stream_token(char)
    #                             await asyncio.sleep(stream_delay)
                            
    #                         await current_tool_msg.update()
                            
    #                         # 記錄工具訊息為活躍等待訊息，以便之後清理
    #                         active_waiting_ids = cl.user_session.get("active_waiting_message_ids") or []
    #                         if current_tool_msg.id not in active_waiting_ids:
    #                             active_waiting_ids.append(current_tool_msg.id)
    #                             cl.user_session.set("active_waiting_message_ids", active_waiting_ids)
                            
    #                         print(f"[ChatManager] 顯示工具呼叫訊息: {tool_name}")
                            
    #                         # 儲存當前工具訊息的來源 agent
    #                         tool_results[agent_name] = {
    #                             "tool_name": tool_name,
    #                             "message": current_tool_msg
    #                         }
    #                         continue
                        
    #                     # 處理工具執行結果 (user_proxy)
    #                     if agent_name == "user_proxy" and current_tool_msg is not None:
    #                         processed_indices.add(i)
                            
    #                         # 找到對應的工具呼叫 agent
    #                         calling_agent = None
    #                         for stored_agent, tool_info in tool_results.items():
    #                             if tool_info["message"] == current_tool_msg:
    #                                 calling_agent = stored_agent
    #                                 break
                            
    #                         if content and isinstance(content, str) and not self._is_system_message(content):
    #                             # 檢查 content 類型
    ...
    #                             # 特殊處理搜索結果
    #                             if calling_agent == "search_agent" and isinstance(content, dict) and "ranked_documents" in content:
    #                                 # 保存搜索結果到 session，讓 SummaryAgent 使用
    #                                 import chainlit as cl
    #                                 cl.user_session.set("latest_search_results", content)
    #                                 cl.user_session.set("search_query", tool_results[calling_agent]["tool_name"])
                                    
    #                                 # 轉給 SummaryAgent 處理
    #                                 print(f"[ChatManager] 搜索完成，轉給 SummaryAgent 生成摘要")
    #                                 # 不顯示搜索結果，直接轉給 summary_agent
    #                                 current_tool_msg = None
    #                                 continue
    #                             # 特殊處理深入分析結果 - 直接顯示在聊天中
    #                             elif calling_agent == "deep_analysis_agent" and isinstance(content, str) and ("📊 深入分析報告" in content or "⚠️ 需要變更" in content or "✅ 維持現狀" in content):
    #                                 # 深入分析結果直接顯示在聊天中
    #                                 await current_tool_msg.stream_token(f"✅ **{tool_results[calling_agent]['tool_name']} 執行完成**\n\n")
    #                                 for char in content:
    #                                     await current_tool_msg.stream_token(char)
    #                                     await asyncio.sleep(stream_delay)
    ...
    #                                 )
    #                                 current_tool_msg.actions = [upload_action]
    #                                 await current_tool_msg.update()
                                    
    #                                 # 儲存該訊息對應的 action ids
    #                                 action_ids = [upload_action.id]
    #                                 action_map = cl.user_session.get("action_map") or {}
    #                                 action_map[current_tool_msg.id] = action_ids
    #                                 cl.user_session.set("action_map", action_map)
                                    
    #                                 print(f"[ChatManager] 已為深入分析結果添加上傳按鈕")
    #                             else:
    #                                 # 一般工具結果處理
    #                                 completion_msg = f"✅ **{tool_results.get(calling_agent, {}).get('tool_name', '工具')} 執行完成**\n\n📄 結果已儲存，點擊右側查看詳細內容"
    #                                 current_tool_msg.content = completion_msg
    #                                 await current_tool_msg.update()

    #                                 try:
    #                                     # 把 element 綁到剛剛的 message (for_id)
    #                                     text_elem = cl.Text(
    #                                         name=f"{tool_results.get(calling_agent, {}).get('tool_name', 'tool')}_result",
    #                                         content=content_str,
    #                                         display="side"
    #                                     )
    #                                     await text_elem.send(for_id=current_tool_msg.id)
    #                                 except Exception as e:
    #                                     print(f"[ChatManager] 無法送出 element: {e}")
                            
    #                         current_tool_msg = None
    #                         print(f"[ChatManager] 工具執行完成，結果已處理")
    #                         continue
                        
    #                     ## === 檢查是否為等待確認或內容的訊息 ===
    #                     if agent_name == "host_agent" and isinstance(content, str) and any(tag in content for tag in ["[等待法條確認]", "[等待案例確認]", "[等待深入分析確認]", "[等待法條內容]", "[等待案例內容]", "【等待法條確認】", "【等待案例確認】", "【等待深入分析確認】", "【等待法條內容】", "【等待案例內容】"]):
    #                         processed_indices.add(i)
    ...
    #                         if needs_buttons:
    #                             # 根據標記決定按鈕類型
    #                             if "[等待法條確認]" in content or "【等待法條確認】" in content:
    #                                 confirm_label = "✅ 是，搜索法條"
    #                                 cancel_label = "❌ 否，取消"
    #                                 confirm_action = "confirm_legal_search"
    #                                 cancel_action = "cancel_legal_search"
    #                             elif "[等待案例確認]" in content or "【等待案例確認】" in content:
    #                                 confirm_label = "✅ 是，搜索案例"
    #                                 cancel_label = "❌ 否，取消"
    #                                 confirm_action = "confirm_case_search"
    #                                 cancel_action = "cancel_case_search"
    #                             elif "[等待深入分析確認]" in content or "【等待深入分析確認】" in content:
    ...
    #                         print(f"[ChatManager] 開始串流顯示訊息 [{i}] (等待回應)")
    ...
    #                         # 儲存訊息 ID 和內容（即使是等待訊息也要保存，以便之後添加按鈕）
    #                         cl.user_session.set("_last_agent_message_id", cl_msg.id)
    #                         cl.user_session.set("_last_agent_message_content", formatted_content)
    ...
    #                                 try:
    #                                     if cl.user_session.get("analysis_confirmed"):
    #                                         already_confirmed = True
    #                                 except Exception:
    #                                     pass
                                
    #                             # 檢查此消息是否已被清除（搜尋/分析已確認）
    #                             cleaned_msg_ids = cl.user_session.get("_cleaned_message_ids") or []
    #                             if cl_msg.id in cleaned_msg_ids:
    #                                 print(f"[ChatManager] 消息 {cl_msg.id} 已在清除列表中，不添加按鈕")
    #                                 cl_msg.content = formatted_content
    #                                 await cl_msg.update()
    #                                 print(f"[ChatManager] 完成顯示訊息 [{i}] (已清除)")
    #                                 continue
                                
    #                             if already_confirmed:
    #                                 print(f"[ChatManager] analysis already confirmed in session, 不再顯示確認按鈕")
    #                                 # 直接顯示已處理的 host 內容，不附加按鈕
    #                                 cl_msg.content = formatted_content
    #                                 await cl_msg.update()
    #                                 print(f"[ChatManager] 完成顯示訊息 [{i}] (已確認)")
    #                                 continue
    ...
    #                                 cl.user_session.set("action_map", action_map)
    #                                 print(f"[ChatManager] 已紀錄 actions for message {cl_msg.id}: {action_ids}")
    ...
    #                                     cl.user_session.set("active_waiting_message_ids", active_waiting_ids)
    #                             except Exception as e:
    #                                 print(f"[ChatManager] 無法記錄 action ids: {e}")
                            
    #                         print(f"[ChatManager] 完成顯示訊息 [{i}] 並等待使用者回應")
    ...
    #                         processed_indices.add(i)
    #                         print(f"[ChatManager] 跳過系統訊息")
    #                         continue
                        
    #                     # === 新增：只顯示關鍵 Agent 的訊息 ===
    #                     # 只顯示: host_agent, deep_analysis_agent, case_analyst, legal_analyst, report_generator
    #                     display_agents = ["host_agent", "summary_agent", "deep_analysis_agent", "constraint_customization_agent", "case_analyst", "legal_analyst", "report_generator", "code_executor"]
                        
    #                     # 調試：如果是 constraint_customization_agent，檢查為什麼被跳過
    #                     if agent_name == "constraint_customization_agent":
    #                         print(f"[ChatManager] 🔍 constraint_customization_agent 檢查:")
    #                         print(f"   - 在 display_agents 中: {agent_name in display_agents}")
    #                         print(f"   - content 是否為空: {not content or not content.strip()}")
    #                         print(f"   - 是否為系統消息: {self._is_system_message(content) if content else 'N/A'}")
                        
    #                     if agent_name not in display_agents:
    #                         processed_indices.add(i)
    #                         print(f"[ChatManager] 跳過非關鍵 Agent: {agent_name}")
    #                         continue
                        
    #                     if agent_name == "code_executor":
    #                         if "EXECUTION_COMPLETE" not in content:
    #                             processed_indices.add(i)
    #                             print(f"[ChatManager] 跳過 code_executor 中間過程")
    #                             continue
                        
    #                     # 🔄 在顯示新訊息前，清理所有活躍的等待訊息按鈕
    #                     await self._cleanup_active_waiting_messages()
                        
    #                     # 標記為已處理
    #                     processed_indices.add(i)
                        
    #                     # ⭐ 新增：如果是 summary_agent 或搜索相關的消息，嘗試提取 case_id
    #                     if agent_name in ["summary_agent", "search_agent"] and isinstance(content, str):
    #                         extracted_case_id = self._extract_and_store_case_id(content)
    #                         if extracted_case_id:
    #                             print(f"[ChatManager] 📌 已成功提取並存儲 case_id: {extracted_case_id}")
                        
    #                     # 創建新的 Chainlit 訊息並串流
    #                     emoji = self._get_emoji_for_agent(agent_name)
    #                     formatted_content = f"{emoji} **{agent_name}**\n\n{content}"
                        
    #                     print(f"[ChatManager] 開始串流顯示訊息 [{i}]")
                        
    #                     # ⭐ 特殊處理：constraint_customization_agent 的消息
    #                     if agent_name == "constraint_customization_agent":
    #                         print(f"[ChatManager] 🎯 constraint_customization_agent 消息將被顯示")
    ...
    #                     await cl_msg.update()
    #                     print(f"[ChatManager] 完成顯示訊息 [{i}]")
                        
    #                     # 如果是 deep_analysis_agent 的報告，添加上傳按鈕
    #                     if agent_name == "deep_analysis_agent" and ("📊 深入分析報告" in content or "⚠️ 需要變更" in content):
    #                         print(f"[ChatManager] 檢測到深入分析報告，添加上傳按鈕")
    #                         upload_action = cl.Action(
    #                             name="upload_analysis_report",
    #                             label="📤 上傳報告",
    #                             payload={"action": "upload"}
    #                         )
    #                         cl_msg.actions = [upload_action]
    #                         await cl_msg.update()
    ...
    #                             action_map[cl_msg.id] = action_ids
    #                             cl.user_session.set("action_map", action_map)
    #                             print(f"[ChatManager] 已為深入分析報告添加上傳按鈕")
    #                         except Exception as e:
    #                             print(f"[ChatManager] 無法記錄 action ids: {e}")
                        
    #                     # 儲存最後一條訊息的 ID 和內容以便稍後添加按鈕
    #                     cl.user_session.set("_last_agent_message_id", cl_msg.id)
    #                     cl.user_session.set("_last_agent_message_content", formatted_content)
    ...
    #         print(f"[ChatManager] 對話任務完成")
            
    #         # ⭐ 確保 GroupChat.messages 沒有被誤清空
    #         # 由於 initiate_chat 可能有某些內部行為，我們需要驗證訊息是否被保留
    #         if len(self.group_chat.messages) < start_message_count:
    #             print(f"[ChatManager] ⚠️ 警告：GroupChat.messages 被清空了！（原: {start_message_count}, 現: {len(self.group_chat.messages)}）")
    #             print(f"[ChatManager] 嘗試恢復已保存的訊息...")
    #             self.group_chat.messages = saved_messages
    #             print(f"[ChatManager] ✅ 已恢復 {len(saved_messages)} 條訊息")
    #         else:
    #             print(f"[ChatManager] ✅ GroupChat.messages 保持正常（訊息數: {len(self.group_chat.messages)}）")
            
    #         # ⭐ 等待消息被添加到 group_chat
    #         # constraint_customization_agent 的消息可能在 chat_task 完成後才被添加
    #         prev_count = len(self.group_chat.messages)
    #         print(f"[ChatManager] 開始等待消息被添加，當前訊息數: {prev_count}")
            
    #         # 特別檢查：是否存在 constraint_customization_agent 的消息
    #         has_constraint_agent_msg = any(
    #             msg.get("name") == "constraint_customization_agent" 
    #             for msg in self.group_chat.messages
    #         )
    #         print(f"[ChatManager] 檢查是否存在 constraint_customization_agent 消息: {has_constraint_agent_msg}")
            
    #         # 如果沒有 constraint_customization_agent 消息，持續等待
    #         wait_for_constraint_agent = not has_constraint_agent_msg
            
    #         for wait_count in range(10):  # 最多等待 10 秒 (200 * 0.05)
    #             current_count = len(self.group_chat.messages)
    #             if current_count > prev_count:
    #                 print(f"[ChatManager] 檢測到新消息被添加: {prev_count} -> {current_count}")
    #                 prev_count = current_count
                    
    #                 # 再次檢查是否有 constraint_customization_agent 消息
    #                 if wait_for_constraint_agent:
    #                     has_constraint_agent_msg = any(
    #                         msg.get("name") == "constraint_customization_agent" 
    #                         for msg in self.group_chat.messages
    #                     )
    #                     if has_constraint_agent_msg:
    #                         print(f"[ChatManager] ✅ 找到 constraint_customization_agent 消息，停止等待")
    #                         wait_for_constraint_agent = False
    #                         break
                        
    #             await asyncio.sleep(0.05)
            
    #         # 處理剩餘訊息（只處理任務完成後新產生的訊息）
    #         all_messages = self.group_chat.messages
    #         final_count = len(all_messages)
            
    #         print(f"[ChatManager] 最終訊息數: {final_count}")
    #         print(f"[ChatManager] last_check_count: {last_check_count}, start_message_count: {start_message_count}")
            
    #         # ⭐ 確保處理所有未處理的消息（包括 constraint_customization_agent）
    #         # 關鍵：不依賴 last_check_count，而是檢查所有消息是否已被處理
    #         messages_to_process = []
    #         for i in range(start_message_count, final_count):
    #             if i not in processed_indices:
    #                 messages_to_process.append(i)
            
    #         print(f"[ChatManager] 檢查未處理消息: range({start_message_count}, {final_count})")
    #         print(f"[ChatManager] processed_indices 中已處理的消息: {sorted(processed_indices)}")
            
    #         if messages_to_process:
    #             print(f"[ChatManager] 處理剩餘訊息: {messages_to_process}")
                
    #             for i in messages_to_process:
    #                 if i in processed_indices:
    #                     continue
    ...
    #                 display_preview = content if content is not None else "<None>"
    #                 print(f"[ChatManager] 處理剩餘訊息 [{i}] {agent_name}: {display_preview[:80]}...")
                    
    #                 # ⭐ 調試：追踪所有消息
    #                 if agent_name in ["constraint_customization_agent", "host_agent"]:
    #                     print(f"[ChatManager] 🔍 {agent_name} 消息被檢測到，將被處理")
                    
    #                 # 跳過使用者輸入
    #                 if agent_name == "user_proxy" and content == message:
    #                     processed_indices.add(i)
    #                     print(f"[ChatManager] 跳過剩餘訊息中的 user_proxy 原始訊息")
    #                     continue
                    
    #                 # === 檢查是否為等待確認或內容的訊息 ===
    #                 if agent_name in ["host_agent", "summary_agent"] and isinstance(content, str) and any(tag in content for tag in ["[等待法條確認]", "[等待案例確認]", "[等待深入分析確認]", "[等待法條內容]", "[等待案例內容]", "【等待法條確認】", "【等待案例確認】", "【等待深入分析確認】", "【等待法條內容】", "【等待案例內容】"]):
    #                     processed_indices.add(i)
    ...
    #                     print(f"[ChatManager] 開始串流顯示剩餘訊息 [{i}] (等待回應)")
    ...
    #                             cl_msg.content = formatted_content
    #                             await cl_msg.update()
    #                             print(f"[ChatManager] 完成顯示剩餘訊息 [{i}] (已確認)")
    #                             continue
    ...
    #                             cl_msg.content = formatted_content
    #                             await cl_msg.update()
    #                             print(f"[ChatManager] 完成顯示剩餘訊息 [{i}] (已清除)")
    #                             continue
    ...
    #                     print(f"[ChatManager] 完成顯示剩餘訊息 [{i}] 並等待使用者回應")
    ...
    #                 # 過濾純系統訊息
    #                 is_system = content and self._is_system_message(content)
    #                 if not content or not content.strip() or is_system:
    #                     processed_indices.add(i)
    #                     print(f"[ChatManager] 跳過訊息 [{i}] - 原因: empty={not content}, blank={not content or not content.strip()}, system_msg={is_system}")
    #                     if content:
    #                         print(f"[ChatManager] 訊息前 100 字: {str(content)[:100]}")
    #                     continue
    ...
    #                 print(f"[ChatManager] 開始串流顯示剩餘訊息 [{i}]")
    ...
    #                 await cl_msg.update()
    #                 print(f"[ChatManager] 完成顯示剩餘訊息 [{i}]")
                    
    #                 # 如果是 host_agent 的非等待訊息，清理之前的等待訊息按鈕
    #                 if agent_name == "host_agent" and not any(tag in content for tag in ["[等待法條確認]", "[等待案例確認]", "[等待深入分析確認]", "[等待法條內容]", "[等待案例內容]", "【等待法條確認】", "【等待案例確認】", "【等待深入分析確認】", "【等待法條內容】", "【等待案例內容】"]):
    #                     active_waiting_ids = cl.user_session.get("active_waiting_message_ids") or []
    ...
    #         # 更新追蹤計數
    #         self.last_processed_count = final_count
            
    #         # 保存完整對話歷史
    #         for i in range(start_message_count, final_count):
    #             if i < len(all_messages):
    #                 self.full_conversation_history.append(all_messages[i])
            
    #         print(f"[ChatManager] 更新 last_processed_count: {self.last_processed_count}")
    #         print(f"[ChatManager] 總歷史訊息數: {len(self.full_conversation_history)}")
    #         print(f"[ChatManager] ===== 對話結束 =====\n")
    #         cl.user_session.set("_cleaned_message_ids", [])
    #         print(f"[ChatManager] 清空 _cleaned_message_ids")
    #         # 🔄 對話完成前，最後清理一次所有活躍等待訊息
    #         await self._cleanup_active_waiting_messages()
    #         # 處理最終結果
    #         result = await self._process_chat_result(chat_result)
    ...
    #         print(f"[ChatManager] Error: {error_detail}")
            
    #         error_msg = f"❌ 對話過程發生錯誤: {str(e)}"
    ...
    async def _stream_normal_agent_message(self, agent, content, delay):
        ...
        if agent == "deep_analysis_agent" and ("📊" in content or "改善措施" in content or "推薦狀態" in content):
            cl.user_session.set("current_analysis_report", content)
            print(f"[ChatManager] ✅ 已保存深入分析報告到 session（來自 agent message），長度: {len(content)} 字符")
        
        if agent == "constraint_customization_agent" and ("重新求解" in content or "新模型" in content or "變數已更新" in content):
            cl.user_session.set("current_analysis_report", content)
