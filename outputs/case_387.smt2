...
; Case ID: case_387
; Generated at: 2025-10-21T08:38:55.106789
;
...
(declare-const capital_minimum_met Bool)
...
(declare-const required_min_net_worth_ratio Real)
(declare-const required_min_ratio Real)
(declare-const risk_capital Real)
(declare-const semiannual_net_worth_audited Bool)
...
(assert (= net_worth_ratio (/ net_worth risk_capital)))

; [insurance:capital_level] 資本等級分類（1=適足, 2=不足, 3=顯著不足, 4=嚴重不足）
(assert (let ((a!1 (or (not (<= 0.0 net_worth))
               (not (<= (* (/ 1.0 4.0) required_min_ratio)
                        capital_adequacy_ratio))))
      (a!2 (ite (and (>= capital_adequacy_ratio required_min_ratio)
                     (not (<= required_min_ratio capital_adequacy_ratio)))
                1
                0)))
  (= capital_level (ite a!1 4 a!2))))

; [insurance:capital_minimum_met] 自有資本與風險資本比率及淨值比率均不低於一定比率
(assert (= capital_minimum_met
   (and (>= capital_adequacy_ratio required_min_ratio)
        (>= net_worth_ratio required_min_net_worth_ratio))))
...
(assert (= reporting_semiannual_compliance
   (and semiannual_report_submitted
        ...
        semiannual_net_worth_audited)))

; [insurance:reporting_compliance] 依規定申報資本等級相關資訊
...
; [meta:penalty_conditions] 處罰條件：資本比率或淨值比率低於一定比率，或未依規定申報資本等級相關資訊時處罰
(assert (= penalty (or (not capital_minimum_met) (not reporting_compliance))))

; ============================================================
; Facts (Case Specific)
; ============================================================

(assert (= own_capital (/ -54303.0 100.0)))
(assert (= risk_capital 100))
(assert (= net_worth -100))
(assert (= required_min_ratio 2.0))
(assert (= required_min_net_worth_ratio 0.0))
...
(assert (= semiannual_net_worth_audited false))
